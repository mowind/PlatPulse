...
describe('PAGE-ADMIN-AGENTS (Agent lifecycle)', () => {
  ...
  it('keeps the secret out of URL state and discards it when the view is left', async () => {
    const secret = 'pp_enroll_0195f2a1-0031-4031-8031-000000000031_abc123def456'
    mockFetch({
      '/api/public/v1/session': () => jsonResponse(OWNER_SESSION, 200),
      '/api/admin/v1/agents/enroll-token': () =>
        jsonResponse(
          {
            token_id: '0195f2a1-0031-4031-8031-000000000031',
            token: secret,
    ...
    // The secret appears exactly once in the success response.
    expect(await screen.findByText(secret)).toBeTruthy()
    expect(screen.getAllByText(/shown exactly once/).length).toBeGreaterThan(0)
    // It is not placed in URL/query state or browser history.
    expect(window.location.href).not.toContain(secret)
    expect(JSON.stringify(window.history.state)).not.toContain(secret)
    // The redacted Audit link/context and request id are present.
    expect(screen.getByText(/enrollment_token_created/)).toBeTruthy()
    expect(screen.getAllByText(/request req-enroll/).length).toBeGreaterThan(0)
    // The redacted Audit link navigates to the Audit review surface.
    expect(screen.getByRole('link', { name: 'review the redacted Audit log' })).toBeTruthy()

    // Leaving the success view unmounts it; the secret is not recoverable.
    await act(async () => {
  ...
  it('surfaces typed mutation errors on the enroll form', async () => {
    mockFetch({
      '/api/public/v1/session': () => jsonResponse(OWNER_SESSION, 200),
      '/api/admin/v1/agents/enroll-token': () => errorBody('invalid_lifetime', 400),
    })
    renderAt('/admin/agents/enroll')
    ...
    expect(alert.textContent).toContain('invalid_lifetime')
    // The draft form survives the failure.
    expect(screen.getByRole('button', { name: 'Create enrollment token' })).toBeTruthy()
  })
})
...
describe('PAGE-ADMIN-AGENT-RECOVER and PAGE-ADMIN-AGENT-ROTATE', () => {
  it('creates a one-time recovery token and explains the Epoch advance', async () => {
    const secret = 'pp_recover_0195f2a1-0041-4041-8041-000000000041_deadbeef'
    mockFetch({
      '/api/public/v1/session': () => jsonResponse(OWNER_SESSION, 200),
      [`/api/admin/v1/agents/${AGENT_ID}`]: () => jsonResponse(AGENT_DIAGNOSTIC, 200),
      [`/api/admin/v1/agents/${AGENT_ID}/recover`]: () =>
        jsonResponse(
          {
            agent_id: AGENT_ID,
            agent_epoch: 1,
            token_id: '0195f2a1-0041-4041-8041-000000000041',
            token: secret,
            expires_at: '2026-08-13T08:00:00Z',
    ...
    renderAt(`/admin/agents/${AGENT_ID}/recover`)

    await screen.findByRole('heading', { level: 1, name: /Recover Agent 0195f2a1/ })
    expect(await screen.findByText(/Current Agent Epoch:/)).toBeTruthy()
    // PATTERN-CONFIRMATION: the high-risk action needs the typed phrase.
    expect(
      screen.getByRole('button', { name: 'Create recovery token' }),
    ).toHaveProperty('disabled', true)
    fireEvent.click(screen.getByLabelText(/I understand: recovery advances/))
    fireEvent.click(screen.getByRole('button', { name: 'Create recovery token' }))

    expect(await screen.findByText(secret)).toBeTruthy()
    // Recovery advances the Epoch without a duplicate Agent.
    expect(screen.getByText(/Epoch advances from 1 to 2/)).toBeTruthy()
    expect(screen.getByText(/never duplicated/)).toBeTruthy()
    expect(window.location.href).not.toContain(secret)
    // The mutation links to the redacted Agent audit trail.
    expect(screen.getByRole('link', { name: 'view Agent audit' })).toBeTruthy()
  })

  it('rotates with an explicit overlap window and optional immediate revocation', async () => {
    const newSecret = 'pp_agent_0195f2a1-0051-4051-8051-000000000051_c0ffee'
    const fetchMock = mockFetch({
      '/api/public/v1/session': () => jsonResponse(OWNER_SESSION, 200),
      [`/api/admin/v1/agents/${AGENT_ID}`]: () => jsonResponse(AGENT_DIAGNOSTIC, 200),
      [`/api/admin/v1/agents/${AGENT_ID}/credentials/rotate`]: () =>
        jsonResponse(
          {
            agent_id: AGENT_ID,
            credential_id: '0195f2a1-0051-4051-8051-000000000051',
            credential: newSecret,
            created_at: '2026-08-12T08:00:00Z',
            overlap_hours: 24,
            revoke_after: '2026-08-13T08:00:00Z',
            revoked_previous_ids: [],
            overlap_credential_ids: [CREDENTIAL_ID],
            request_id: 'req-rotate',
          },
          200,
        ),
    })
    renderAt(`/admin/agents/${AGENT_ID}/rotate`)

    await screen.findByRole('heading', { level: 1, name: /Rotate credential/ })
    // PATTERN-CONFIRMATION: the security mutation needs the typed phrase.
    expect(
      screen.getByRole('button', { name: 'Rotate credential' }),
    ).toHaveProperty('disabled', true)
    fireEvent.click(screen.getByLabelText(/I understand: rotation issues/))
    ...
    await waitFor(() => {
      ...
      expect(calls.some((url) => url.includes('/credentials/rotate'))).toBe(true)
    })
    expect(await screen.findByText(newSecret)).toBeTruthy()
    expect(screen.getByText(/overlap 24 hours/)).toBeTruthy()
    expect(screen.getByText(/stay valid until/)).toBeTruthy()
    expect(screen.getByText(/Agent Epoch was not changed/)).toBeTruthy()
    expect(window.location.href).not.toContain(newSecret)
  })

  it('rotates with explicit old-credential revocation as a distinct option', async () => {
    const newSecret = 'pp_agent_0195f2a1-0051-4051-8051-000000000051_c0ffee'
    mockFetch({
      '/api/public/v1/session': () => jsonResponse(OWNER_SESSION, 200),
      [`/api/admin/v1/agents/${AGENT_ID}`]: () => jsonResponse(AGENT_DIAGNOSTIC, 200),
      [`/api/admin/v1/agents/${AGENT_ID}/credentials/rotate`]: ({ body }) => {
        ...
        return jsonResponse(
          {
            ...
            credential: newSecret,
            created_at: '2026-08-12T08:00:00Z',
            overlap_hours: parsed.overlapHours,
            revoke_after: null,
            revoked_previous_ids: parsed.revokePrevious ? [CREDENTIAL_ID] : [],
            overlap_credential_ids: parsed.revokePrevious ? [] : [CREDENTIAL_ID],
            request_id: 'req-rotate',
          },
          200,
        )
    ...
    await screen.findByRole('heading', { level: 1, name: /Rotate credential/ })
    fireEvent.click(screen.getByLabelText('Revoke the previous credential immediately'))
    fireEvent.click(screen.getByLabelText(/I understand: rotation issues/))
    fireEvent.click(screen.getByRole('button', { name: 'Rotate credential' }))

    expect(await screen.findByText(/revoked immediately/)).toBeTruthy()
    expect(screen.getByText(/revoked: 0195f2a1-0021/)).toBeTruthy()
    expect(screen.queryByText(/stay valid until/)).toBeNull()
    expect(window.location.href).not.toContain(newSecret)
  })
})
