...
expect_security_error guest-admin 401 auth_required "$BASE_URL/api/admin/v1/access-mode"
expect_security_error viewer-admin 403 owner_required -b "$VIEWER_COOKIE" "$BASE_URL/api/admin/v1/access-mode"
expect_security_error viewer-agent 401 agent_auth_required -b "$VIEWER_COOKIE" "$BASE_URL/api/agent/v1/time"
expect_security_status owner-admin 200 -b "$OWNER_COOKIE" "$BASE_URL/api/admin/v1/access-mode"
...
printf '{"mode":"private","confirmed":true}' > "$RUN_ROOT/security-valid-access.json"
expect_security_error missing-csrf 403 csrf_validation_failed -b "$OWNER_COOKIE" -H 'Content-Type: application/json' --data-binary "@$RUN_ROOT/security-valid-access.json" -X PUT "$BASE_URL/api/admin/v1/access-mode"
printf '{not-json' > "$RUN_ROOT/security-malformed.json"
expect_security_error malformed-human-json 400 invalid_json -b "$OWNER_COOKIE" -H 'Content-Type: application/json' -H "Origin: $BASE_URL" -H "X-CSRF-Token: $OWNER_CSRF" --data-binary "@$RUN_ROOT/security-malformed.json" -X PUT "$BASE_URL/api/admin/v1/access-mode"
...
expect_security_error agent-credential-admin 401 auth_required -H "Authorization: Bearer $AGENT_CREDENTIAL" "$BASE_URL/api/admin/v1/access-mode"
