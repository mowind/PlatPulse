...
fail() { printf 'Release-candidate harness: FAIL (%s)\n' "$1" >&2; exit 1; }
for command in cargo curl jq mktemp python3 tar; do command -v "$command" >/dev/null 2>&1 || unavailable "missing required command: $command"; done
...
"$SERVER" network create --config "$CONFIG" --key platon-mainnet --display-name 'PlatON Mainnet' --genesis-hash "0x$(printf '0%.0s' {1..64})" --chain-id 210425 --p2p-network-id 210425 --address-hrp lat >>"$CLI_LOG" 2>&1 || fail 'Network provisioning failed; see preserved CLI log'
"$SERVER" network create --config "$CONFIG" --key platon-testnet --display-name 'PlatON Testnet' --genesis-hash "0x$(printf '1%.0s' {1..64})" --chain-id 210426 --p2p-network-id 210426 --address-hrp lat >>"$CLI_LOG" 2>&1 || fail 'Network provisioning failed; see preserved CLI log'
...
request_id() { awk -F': ' 'tolower($1) == "x-request-id" { gsub("\\r", "", $2); print $2; exit }' "$1"; }
request() {
  local name="$1"; shift
...
jq -e --arg report_id "$REPORT_ID" '.receipt.report_id == $report_id and (.receipt.disposition == "accepted" or .receipt.disposition == "partially_accepted")' "$RUN_ROOT/report.body" >/dev/null || fail "Report Receipt was not committed (request $LAST_REQUEST_ID)"
