...
def run_qualification(profile_path: Path, output_root: Path) -> int:
    ...
    try:
        ...
        soak_ok = soak_stats["sent"] > 0
        scenarios.append(scenario("sustained_report_load", "PASS" if soak_ok else "FAIL", f"submitted {soak_stats['sent']} reports during the observation window ({soak_stats['failures']} response failures included in the global error-rate threshold)"))
        if not soak_ok:
            request_failures += 1
        rest_stop.set()
