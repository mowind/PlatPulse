...
| 2 | Home top row fills Emerald's 3×2 six-tile shape with PlatPulse's own six metrics — Active Nodes, Healthy Nodes, Cumulative blocks, Attention, Networks, Cumulative rewards — instead of Emerald's resource tiles. | Upstream's "remaining value" tile has no PlatPulse field (no price/expiry/plan/billing data exists), and summing Host memory/disk/traffic across Nodes would double-count Hosts shared by several Nodes. The two cumulative cards use the Server's per-Network deduplicated Validator summaries; their Cross-Network Validator Overview adds those native-unit numbers without asset conversion, prints cumulative block counts complete, keeps the abbreviated reward tile, and prints each metric's actual scope, coverage denominator and Partial marker on the card face, with exact values plus per-Network coverage still available through each card's Breakdown. Layout, card shell, typography and spacing follow upstream. |
...
| 11 | The Home statistics slot keeps Emerald's six cards in a 3×2 grid, but at ≥1024px it takes roughly 5:6 against the map (the map is the larger track) without the pinned 37:61 ratio or a fixed 232px band; below that the complete 2:1 map comes first (upstream's DOM order) and the six cards follow it. | The approved overview needs room for three columns of equal cards while the complete world map stays visible, and the original fixed band cannot hold either. The 5:6 split gives the map the larger half of the band as the reference lays it out — the map column then fills that width and grows with it — while three equal statistics columns still fit; when the six tiles are shorter than the map band they sit on its floor, so the gap down to the Network group keeps the same 16px rhythm as the Node cards; the tile keeps the earlier four-card grid's compact height: a 44px header row holds the title and the metric marker or Breakdown control, and the value sits beneath it over a shared one-line stat slot with no footer row, so the two cumulative tiles can print their scope/coverage line without breaking the six-tile height or value baseline. The page keeps its 1280px ceiling and the Node grid's auto-fill minimum track (**recalibrated in deviation 23**). Slot position, card shell and typography are upstream's. |
...
| 19 | Both map layers set only `left/top/width`, and the `map` series keeps its polygons silent (`tooltip.show: false`), exactly as upstream does. From `xl` up the map track is upstream's fixed 22rem (352px) band instead of a proportional track; below `xl` it stays proportional (2:1), so phones and tablets keep the compact map the mobile acceptance measured. | An explicit `height` or `preserveAspect: 'contain'` defers to the container's aspect, which shrank or stretched the world; a bare `-translate-y-1/6` on a canvas as tall as the band would leave the lower ~60px of the band empty at PlatPulse's column width. Upstream's own option — width only, height from the world's aspect ratio — needs only a band tall enough to hold the world: at Home's 1280px ceiling the map column is 672px, so the world is ~347px tall and the fixed 352px band holds it whole, where the previous 2:1 track (336px) clipped ~5px off each pole. The band applies from `xl` because that is where Home reaches its ceiling and the column stops changing; below that a fixed band would overrun the narrower column. The **37:61 tracks and 232px summary band remain superseded by deviation 11**. Regression: `src/components/mapChartLayout.test.ts` (world fills the canvas width, is vertically centred, and the two layers agree); `e2e/map-size.spec.ts` and `e2e/home-geo-map.spec.ts` check the fixed band and painted browser pixels at 1280/1440/2390px. |
| 20 | The Home refinement round compresses the Node identity block (the Node-name anchor keeps its measured 44px hit area through a negative block margin, and the header Role chip reads `Node: Non-validator` on one line), shares each Linked Validator metric pair's grid rows through `subgrid` (one CSS source, no JSX restatement), shortens only the `Validator status unknown` chip to a real 44px `Unknown` control with a focusable explanation, converges an authoritative empty Validator with no metric value into a compact line even when the only other status is `Unranked` (decided from raw fields and the identity/request state, never a formatted string), keeps the six overview values on one baseline by top-aligning each value under the uniform header, and drops the last dotted label/value leader on the Speed row. **The authoritative-empty Validator presentation is adjusted by deviation 23.** | Emerald has no Linked Validator, staking-identity or Unranked concept, and its Node card has more vertical room than the PlatPulse card needs. The refinements keep PlatPulse's own evidence semantics (a ranking status is not a metric value; Provider failure is never a no-live-Validator result) while adopting Emerald's compact spacing and label-over-value anatomy. Regression: unit tests for the fold predicates and the summary caption, `e2e/home-convergence.spec.ts`, `e2e/validator-summary.spec.ts`, `e2e/node-business-metrics.spec.ts` and `e2e/linked-validator.spec.ts` at the fixed viewports. The Linked Validator subgrid/six-stacked-cell anatomy is **superseded by deviation 21**. |
| 21 | The Home Node card keeps the emphasized label-over-value form only for the cumulative metrics (Cumulative blocks/rewards, and the chain heights when the card is narrow). Every ordinary parameter — chain heights on a wide card, Txs/Peers, CPU/Memory/Node data/Speed, and the four Linked Validator parameters — is a compact left-label/right-value key-value line with no per-row frame, no dotted leader and no fixed two-line label height. A named `node-card` container query on the card's own width switches each group between two columns and one full-width line per metric, and `wideValue` moves the Node data used/total caption onto the title line on a wide card. The two Linked Validator cumulative cells stay two columns at every width, a narrow card gives the four parameters four full-width lines (all default-visible), short display names keep their full field name as an accessible name and in Details, and the identity / Provider Data state / identifier / status dimensions are unchanged. | Emerald's Node card has more vertical room and no Linked Validator, so its exact anatomy cannot be transplanted. This round keeps the reference's compact, high-density key-value rhythm without hiding a field or reducing type. Measured on the seeded convergence fixtures (Chromium, no viewport scaling): the ordinary validator card falls from 665.5px to 546.0px (**−18.0%**) at a 362px card and from 619.2px to 496.0px (**−19.9%**) for the healthy Node-A card. At the 303px card the desktop-1280 four-column grid produces, the mandated four full-width chain rows and four full-width parameters offset the compaction, so the card stays within about ±3% (681.5 → 682.0px for Node H; 651.2 → 667.2px for Node A) rather than shrinking by cutting fields. The width switch is a CSS container query, never a viewport breakpoint or a JavaScript size listener. Regression: `e2e/node-business-metrics.spec.ts` (forced 300/360/560px container-query switching), `e2e/linked-validator.spec.ts`, `e2e/home-convergence.spec.ts`, `e2e/emerald-refinement.spec.ts`, plus the `MetricRow`, `HomeDashboard` and `LinkedValidator` unit suites. **The desktop grid geometry and the switch's minimum track are recalibrated by deviation 23.** |

| 22 | The Home resource progress bars (CPU, Memory, Node data) pass the theme's `info` fill instead of `ProgressThin`'s default `bg-primary` (**remapped to the normal-state `success` token by deviation 23**); the Info/Copy/Details icon controls share one 14px icon size; the chain/parameter grid also falls back to full-width rows when a formatted number cannot fit a compact cell; and a compact Linked Validator parameter states `Unknown` instead of the em-dash placeholder. | `bg-primary` is white in the dark theme, so every ordinary resource bar read as an empty/white track. PlatPulse owns no occupancy percentage threshold rule, so none was invented — the bars use a theme colour rather than an invented success/warning/error condition, and a Server-owned rule can replace it later. The value-length fallback is still width-driven: a 21-digit safe integer cannot fit a compact cell at any card width, and the spec forbids shrinking, splitting or clipping it. `Unknown` is the explicit vocabulary the chain rows already use and never reads as zero; its sanitized reason stays announced. The reference capture's conspicuous Info frame was audited in the browser — the control is transparent by default (no background, border, outline or shadow), accent-tinted on hover, and paints the shared 3px focus-visible ring, which is kept because it is required keyboard feedback. Regression: `e2e/node-business-metrics.spec.ts`, `e2e/emerald-refinement.spec.ts`, and the `MetricRow`/`HomeDashboard`/`LinkedValidator` unit suites. |
| 23 | The Home Node-card density round recalibrates the auto-fill minimum track from 300px to the same 22.5rem (360px at the default root) that the card's own `node-card` container query uses, so the desktop-1280 content column renders three 408px-wide cards instead of four 303px-wide ones; on a wide card the consensus grid is two columns by three rows (Head/QC, Locked/Committed, Txs/Peers) and the four Linked Validator parameters are two columns by two rows, while a card below the track keeps one full-width row per metric. The authoritative-empty Linked Validator card keeps its `No live Validator` and `Unranked` conclusions on a stable Provider-data status row (the long explanation stays in Details), the heading and the status cluster each occupy their own row so the status can never strand right-aligned on a line of its own, and the ordinary resource progress bars pass the normal-state `success` token instead of `info`. | At the 300px minimum the auto-fill grid produced four 303px cards at desktop-1280/1440/1920, below the card's own 22.5rem two-column switch, so both parameter groups rendered as single-column tables — five consensus rows and five Linked Validator rows per card. Aligning the grid minimum with the switch makes the width decision self-consistent (auto-fill can never produce a track narrower than the two-column grids need); desktop-1280 resolves to three wide cards, and four columns return only when the content column can hold four 22.5rem tracks, which the 1280px-capped Home content does not reach. Measured on the seeded Public fixtures (Chromium, no viewport scaling, 408px card): the consensus group falls from 196px/5 rows to 136px/3 rows and the Linked Validator group from 176px/5 rows to 100px/3 rows; the no-validator card falls 519→443px and the linked-validator card 684→517px. The switch stays the card's own container query, never a viewport breakpoint or a JS size listener, and the value-length fallback (deviation 22) still keeps an over-long safe integer untruncated. Regression: `e2e/node-business-metrics.spec.ts` (forced 300/360/560px switching plus a natural-width row and per-cell overflow check at every fixed project), `e2e/home-overview-layout.spec.ts`, `e2e/linked-validator.spec.ts` (the folded empty card), `e2e/home-convergence.spec.ts`, and the `LinkedValidator`/`HomeDashboard` unit suites. |
| 24 | The Home Node-card top-right Validator Activity badge maps PlatScan's Activity to an icon plus the status name and a tone: teal for `verifying`, emerald for `producing`/`active`, and the neutral `muted-foreground` for `observing` and every out-of-scheme value. `active` takes a solid `emerald-700` foreground on light and `emerald-400` on dark, over Producing's unchanged background tint and a lighter border; no glow, solid fill or size change. | Emerald has no PlatScan Validator Activity; the badge is a PlatPulse addition recorded in ADR 0006. The foreground is chosen per theme because the `--success` token (≈emerald-500) fails 4.5:1 at the badge's 11px text on a light surface, while `emerald-400` is only legible on the dark surface, so the earlier single 70%-opacity Emerald was too dark in dark mode. Regression: `ValidatorActivityBadge.test.tsx` asserts the four-state mapping, the neutral Observing tone, one badge geometry and the 14px/2px/`currentColor` glyph. |
...
> **Superseded layout:** the 37:61 tracks and 232px band recorded in this section
> were replaced by the six-card overview in **deviation 11** (4:3 statistics/map
> at ≥1024px, a contained 2:1 map, statistics above the map below that). This
> section remains as the record of that earlier, human-approved composition.
