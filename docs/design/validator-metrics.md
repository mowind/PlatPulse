...
- Both Home Node cards and Node detail expose all six metrics. Normal Home cards use a dedicated two-column, three-row section with stacked label/value cells; mobile does not require hover or expansion. Each pair of cells shares its grid rows, so a single-line label does not reserve a second line while the paired values still align on the taller label; long labels such as `Delegation reward share` wrap at normal word boundaries, and short states such as `Unknown` or `Unranked` are never split. Keep normal-height numerals and span the full metric width for rare full-number values longer than 15 characters rather than shrinking, clipping, or splitting their digits. Node detail keeps full available precision.
