...
- Both Home Node cards and Node detail expose all six metrics. Normal Home cards use a dedicated two-column, three-row section with stacked label/value cells; mobile does not require hover or expansion. Keep normal-height numerals, use whole-word label wrapping, and span the full metric width for rare full-number values longer than 15 characters rather than shrinking, clipping, or splitting their digits. Node detail keeps full available precision.
- The only compact missing-metrics exception is a currently confirmed Not Validator with all six values absent, a fresh successful detail or authoritative empty response, and no ranked/unranked or not-applicable evidence to preserve. It may replace six empty slots with one explicit no-current-staking/no-metrics line. Unknown/verifying/loading/error/unconfigured/stale states, historical values, genuine zero, an authoritative Unranked, and Not applicable never trigger that exception; full states/reasons remain in Details.
