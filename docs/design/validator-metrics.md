...
- Keep the Server-owned Current-Selection Validator Summary grouped by Network. The human-approved Cross-Network Validator Overview is a separate Home display: add the already-deduplicated Network summaries numerically for cumulative blocks and gross cumulative rewards. This supersedes the earlier ban on cross-Network overview sums, not Network-scoped identity or Server aggregate membership. Rewards are native-unit numbers added without asset or fiat conversion; the current DTO does not establish unit identity, so the result is not a single-asset balance or monetary valuation.
...
- These are metrics of linked Validators, not a claim of asset ownership. Aggregates can decrease when filters, effective links, or Active membership change. Current staking validity is not an eligibility filter: an effective Link to a historical/exited identity can still contribute lifetime values.
- Home uses two of its six overview cards for these cumulative sums, following the same Network filter as the original four statistics and map. Cumulative block counts print complete on the tile; reward amounts keep the abbreviated tile form. Add exact decimal strings from Server summaries, never duplicate Node projections or floating-point approximations. Preserve Unknown versus genuine zero, independent known/eligible and stale counts, and partial-known-value subtotals; a missing Network summary makes coverage unavailable rather than zero. Under the value, every cumulative tile prints a directly visible short stat line with its own Network scope, its own coverage denominator and a Partial marker when coverage is incomplete (for example `Mainnet · 3/5 known · Partial`, using the actual result); the two metrics keep independent coverage. This line is part of the shared six-tile stat slot, so all six tiles stay one height and one value baseline.
- Each cumulative card has a keyboard/touch-accessible Breakdown dialog with exact values, every selected Network's independent block/reward coverage and stale counts, linked/eligible counts, and separate unlinked Nodes. This replaces the standalone Validator totals section. A long block count wraps at its thousands separators rather than being clipped, and the Breakdown still exposes every exact digit, the current scope and the Network/native-unit context, including the exact reward amount behind an abbreviated tile. Coverage and scope are never tooltip-only: the face line above carries them, and the Breakdown keeps the full explanation.

## Historical Home aggregation baseline (#159; explicit-Link model)

The following records the earlier delivery, not the current overview layout. The accepted aggregation target above supersedes its no-cross-Network display rule and standalone section; the per-Network Server summary remains unchanged.
...
- Both Home Node cards and Node detail expose all six metrics. Normal Home cards keep only `Cumulative blocks` and `Cumulative rewards` in the emphasized label-over-value form (two columns, no separate inner box); `Network rank`, `Production rate`, `PlatScan 24h rate` and `Delegation reward share` are compact left-label/right-value rows that pair into two columns on a wide card and become four full-width lines when the card is narrow. The card's own width decides through a CSS container query, so mobile does not require hover or expansion and no ordinary label reserves a fixed two-line height. Short display names may be painted, but each keeps its full field name as an accessible name and in Details; short states such as `Unknown` or `Unranked` are never split. Keep normal-height numerals, and fall a group back to full-width rows for rare full-number values too long for a compact cell rather than shrinking, clipping, or splitting their digits. Missing values stay explicit `Unknown`, never zero. Node detail keeps full available precision. See `docs/design/webui.md` rule 6 for the shared card anatomy.
- The compact missing-metrics exception is a currently confirmed Not Validator with no effective metric value, a fresh successful detail or authoritative empty response, and no not-applicable rate to preserve. It replaces six empty slots with one explicit no-current-staking/no-metrics line. Rank status is not a value: an authoritative `unranked` does not prevent compaction and is merged into that line (`Network rank: Unranked`), because it is a ranking outcome rather than a number; `ranked` still requires its value. A partial metric set, a retained historical value, genuine zero, a known not-applicable rate, Unknown/verifying/loading/error/unconfigured/stale states, and an unresolved identity all keep the metric structure. The decision reads the raw fields, the explicit identity verdict and the request state, never a formatted UI string; full states, reasons and diagnostics remain in Details.
- Home keeps a readable Validator name and abbreviated identifier with explicit full-identifier copy and accessible Details controls. Details exposes the exact identifier/values, provenance, update times, and explanations; copy failure offers manual selection rather than silently claiming success. The card remains naturally sized rather than stretching to its tallest neighbour.
- Preserve three independent dimensions: the Node header Role is consensus membership; Current Validator Status is valid staking identity; Provider Data: Current is recent successful collection. Do not substitute one for another. Keep exceptional Provider/identity/rank/staking staleness and counter correction visible; move repetitive explanatory prose to Details.
...
- Earnings use the Network native unit; Home cards may abbreviate (`1.73M`) while the exact amount stays in the accessible Breakdown and Node detail exposes all available source precision without reconstructing digits absent from the source. Cumulative block counts, by contrast, always print the complete number. Source-limited precision is accepted; default displayed percentages use two decimal places.
...
- Validate separate Network sources and unconfigured Network behavior; no cross-Network identity association or merging of the Server-owned summaries. Independently validate the approved Cross-Network Validator Overview's exact numerical sums, missing-summary/partial/zero states, native-unit warning, and per-Network Breakdown. Old manual links must never serve as an automatic-discovery fallback.
