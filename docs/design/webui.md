...
- Validator/Alert/Notification/Retention/Backup/Restore/Doctor/Node Transfer/People/Enrollment/Recovery/Rotation management pages (their Server/API operations exist but are not currently routed); Geo provider selection is routed on Settings, and GeoJS plus a global refresh remain unimplemented;
...
- Geo provider offers only the providers this Server actually implements: Disabled, Local MMDB, and IPinfo. Disabled schedules no lookups at all; Local MMDB resolves countries from the operator-provided GeoLite2 Country database on the Server; IPinfo sends each observed Peer public IP to the fixed third-party endpoint `https://ipinfo.io/{ip}/json` and keeps only the returned country code. GeoJS and a global refresh action are not rendered while they are unimplemented;
- whether a provider sends Peer addresses off the Server is the Server-owned `sends_peer_addresses` flag on each option, so the WebUI never hardcodes which providers do it: a flagged option is labelled as sending observed Peer public IPs to a third party and the card states the destination in plain text before the Owner selects it, together with the fact that only the Owner can enable it, that no upgrade enables it, and that the Server never falls back to another provider;
...
| `SCN-SETTINGS-GEO-PROVIDER` | Through the Geo provider card, select an available provider, submit, and read the audited result plus the refreshed Server status; an unavailable option stays disabled with its reason and is never submitted; IPinfo is offered unselected with its third-party consequence stated, and no unimplemented provider or refresh action appears. A browser scenario never selects an External Geo Provider: the destination is deliberately not configurable, so selecting one would send a Peer address to the real third party. |
| `SCN-GEO-BACKGROUND-RESOLUTION` | With Local MMDB or IPinfo selected, the already-open Public Network view updates its Server-computed Known/Unknown country buckets through the existing realtime invalidation once the background resolution completes, without a reload and without exposing a Peer address, database path, or external request detail. |
...
1. The overview composes the 2x2 statistics block and the map with a stable Grid: about 42% for the statistics (accepted 40–45%) and about 58% for the map (accepted 55–60%), starting at 240–280px tall. State and scope explanations may grow it; the blocks are never cropped or hidden to hold a fixed height, and the layout uses no page-wide `position: absolute`, fixed offsets, or large negative margins.
2. The statistics block keeps the accepted order and semantics: Active Nodes / Healthy Nodes, then Attention / Networks, with one visible line naming its global `All Networks` scope. The filter and the sort control below never change these numbers.
3. The map owns its own region: heading `Peer countries`, the Server-owned Geo state badge (`Starting`/`Empty` for its own load states), and separate lines for the current `Scope`, Peer observation freshness (`Varies by Network` when the in-scope Networks disagree), and the basemap resource state (`Map resource: Starting|Current|Error|Unknown`). Country and Unknown counts, the unknown reasons, the last-good/age/stale metadata, the `n of m Networks` basis note, and the `Partial scope` note all come from the Server's Public Geo Insight; the browser never derives them by subtraction.
4. The count basis is stated in place: Peer records in scope are counted per Node and are never deduplicated by IP, so the same Peer observed on two Nodes is two records. Markers are Server-provided country representative points, never Peer positions or Node deployment locations, and the map never claims unique Peers, Online Nodes, or live Peer presence.
5. Every country with a count keeps an accessible text statistic beside the map, carrying its ISO code and its country name for assistive technology. A country without usable geometry or without a representative point is listed with the reason it is not drawn (`no map outline`, `no representative point`); an unknown country is never drawn, and no marker, random point, or `[0, 0]` fallback is ever fabricated. The per-Node Peer-record basis is stated for every published denominator, including an authoritative zero.
6. The basemap is a fixed, locally hosted Natural Earth 1:110m Admin 0 Countries asset (`platpulse-web/public/assets/geo/world-countries-110m-v1.json`, generated offline by `platpulse-web/scripts/build-world-geometry.mjs`, public domain) served from the Server's own origin. There is no runtime map CDN, tile service, or download. One credit line under the canvas carries the basemap attribution and the Server-provided Geo attribution (`GeoLite Data created by MaxMind` when Local MMDB is selected, the IPinfo credit when IPinfo is selected), and the basemap source label stays in the accessible text. The browser never composes a Geo credit of its own. Geometry is drawn in the asset's own equirectangular space and the representative point is projected into it.
7. The map is transparent: no white card shell, no prominent border or shadow, no large title bar, and no reduced container opacity. Country outlines stay light grey, observed countries take a restrained Emerald fill, and marker labels keep high contrast.
8. Disabled, Empty, Starting, Unknown, Stale, Error, basemap failure, and basemap retry are local states of this region. A Disabled Geo Provider renders the neutral `Peer countries · Disabled by server` notice and never requests geometry; an unavailable basemap states that the counts are unaffected and offers a labelled retry; a render failure is contained by a map-local error boundary. A map or Geo failure never removes the statistics, the toolbar, or a Node card.
9. Narrow layouts stack the 2x2 statistics above the map. The default canvas is the complete world in a compact 150–170px band and the labelled `Show full map` / `Collapse map` control expands it to about 240–280px, restores the compact view instead of hiding the map, and exposes `aria-expanded`/`aria-controls` with a visible focus ring and a 44px touch target.
10. There is no default wheel zoom, no 3D, and no continuous animation; the only transition is the canvas resize, and reduced-motion users get it without motion. Decorative background layers stay pointer-inert, so the map keeps its own pointer interaction.
