...
- the primary Node card uses a compact Komari-inspired density with a neutral one-pixel outline and no coloured edge strip. It shows a two-state Server-owned Node Health marker before the display name/Node ID, the separate `Validator` membership role, process uptime, compact PlatON process CPU/process-memory/Node Data progress, `Head / QC / Locked / Committed / Validator`, process start time, and last Agent report time. The current SPA renders Node Validator Activity nowhere, so neither Home nor the Node hero shows a `Node status` label or an activity badge; Node Validator history remains a Network Overview module. Routine Healthy prose is omitted; an exceptional Server-owned health reason remains visible;
...
- At 768x1024, Home keeps the 2x2 statistics, places the Peer country map above them in reading order, and uses a two-column Node grid at this content width. Node Detail's hero groups fall to two metric cells per row, and its four equal-height metric cards keep their two-by-two deck.
- At 360x800 and 390x844, Home keeps the compact 2x2 statistics, places the compact map above them in reading order, uses a single-column Node grid, and keeps every metric a single full-width `data item / value` row — the metric groups pair only from 30rem, so `Head / Txs / Peers` and the consensus units all stay single-column here. Filter pills scroll within their own control rather than causing page overflow. Node Detail keeps the three resource metrics and three status facts in compact rows, single-column at these widths, stacks the four equal-height metric cards, and keeps the tabs full-width touch controls. At 768x1024 the page composition may be single-column, but the shared table-to-priority-card transform begins below 47.9rem; Admin tables therefore retain their table/local-wrapper behavior at the 768 fixture.
...
- **Public Home, Network, and Node Detail (Issue #147).** The three public surfaces complete the shared foundation without changing routes or information structure. The Home 2x2 statistics, the Network filter/sort controls and Peer/Geo/Validator modules with their independent degradation, the Peer country map scope/denominator/states, the Server-owned Node Health Summary (Healthy green, every other state grey, with the abnormal reason in text), the last-good semantics, and the Node Detail Details/Network tabs with their four charts behave exactly as before; only the visual adaptation changes.
...
| Public Home, Network Overview, and current Node Detail dual-theme adaptation with the calibrated ~60% card surfaces and hover/focus/disclosure feedback | Issue #147, child of Issue #145; visual-only slice of the #146 foundation, keeping the Details/Network tabs and four charts |
