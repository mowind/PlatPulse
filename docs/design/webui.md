...
- Node Health Summary is not a WebUI-computed health color; the two-state Node marker on Home, Network Overview, and Node Detail only reflects the Server-owned health word (§5.4);
...
- selected-Node Public Peer Insight and aggregate Peer History live in a keyboard/touch-operable disclosure with the redaction note; Network Overview shows network-level Peer Insight and Geo/Validator modules separately. No peer address or identity list is exposed;
...
- The shared theme foundation offers Auto, Light, and Dark (see "Theme behavior" below). Light uses a Slate-50-like canvas; Dark uses a coordinated near-#141923 slate canvas with the same restrained Emerald/Lime top atmosphere. Home, Network Overview, public Node Detail, and Login carry one shared top atmosphere: the Emerald default background at `c2c5e88ea19c7cbe18d14a50414e10deca3cc66e` ported faithfully in both themes — the full-viewport neutral base, the emerald/lime atmosphere with its `farthest-side` top radial mask at `opacity: 0.4`, and the low-contrast inclined SVG grid with the upstream `black/40` fill, `black/50` stroke and `mix-blend-overlay` treatment over four local filled cells, plus the Dark variants (`emerald/30`-`lime/30`, `white/2.5` fill, `white/5` stroke, vertical mask). The one deliberate deviation is width: the atmosphere spans the full viewport instead of the upstream fixed 1300px, so it never reads as a partial band on a wide viewport. The layer is decorative, pointer-inert, absent from the accessibility tree, shared rather than per-page, and fades before the lower reading surface. Admin retains the Emerald brand but uses the stable, undecorated workbench background in both themes as specified in §8.5; this exception does not change Home or Login.
- Public display cards follow Emerald default surfaces at `c2c5e88ea19c7cbe18d14a50414e10deca3cc66e`: borderless white in Light and `oklch(0.141 0.005 285.823)` in Dark, both at 60% opacity at rest and opaque on hover-capable devices. Only Home Node cards gain the emerald-600/10 glow and 2px lift; statistics and detail/chart/disclosure cards have no decorative shadow or lift. This does not change the page canvas, atmosphere, layout, internal separators, or semantic status colors. No custom background image or default `backdrop-filter` is introduced. Admin, Login, menus, forms, dialogs, and overlays retain their existing surfaces.
- Public Home, Network Overview, and Node Detail use Emerald’s system font stack (`system-ui, -apple-system, "Segoe UI", Roboto, sans-serif`), including navigation and controls. Card typography follows the corresponding Emerald heading, metric, and responsive detail tiers while retaining tabular numerals and overflow safeguards. No font binaries are copied or downloaded; Admin and Login retain Inter.
...
5. Compact Active Node cards. Each whole root-dashboard card is one semantic link to Node Detail; a two-state Server-owned Node Health marker sits before the Node name and is the card's only status cue. Node Validator Activity is not rendered by the current SPA, so no activity or freshness value puts text on the card. The Network name is plain text and there is no nested Network link. Healthy Nodes omit routine prose; exceptional Nodes may show one short diagnostic line.
6. Card data ownership is explicit: PlatON process `CPU`/`Memory`, `Node data` size/capacity, Host transfer rates (`↑ Up`/`↓ Down`), Node `Head`/`Txs`/`Peers`, and Consensus `QC`/`Locked`/`Committed`/`Validator`. Every metric is one data-item / value unit: the data item sits at the left and the value sits flush right; a unit with progress puts a track under the value line with its explanation below the track, and a unit without progress puts its explanation directly under the value. Inside one semantic group the units pair into two columns wherever a cell still fits the longest label and the longest retained value, and every unit falls back to one full-width row below 30rem. A unit whose explanation can run long — `Node data`, `Peers` — keeps the whole group width. `Node data` reads as a percentage whose explanation gives the used / filesystem-capacity bytes. Missing values remain unavailable rather than becoming zero.
...
1. The overview keeps a stable Grid: about 46% for the 2x2 statistics and 54% for the transparent map, as two real columns that never overlap. No summary card is layered over the band, and no landmass, marker, or corner figure is layered behind a card; the band's left edge carries only a short fade over open water. There is no fixed overview minimum height: removing auxiliary content must also shrink the parent and move filters and Node cards upward. The desktop map footprint follows the band's own proportion at that column width — roughly 270–290px at the 82rem content width — and is neither stretched nor clipped. A small reserved corner-credit strip is allowed; large internal chart margins are not. The transparent Home logo bar is the shell's own row above this band at every viewport: like the Emerald reference, whose map is a content grid item beneath its bar, no part of the map, its markers, or its corner indicator is layered behind the brand, and the band never occupies the bar's strip.
...
7. An Active PlatON Nodes section. Each compact Network Overview card presents a two-state Server-owned Node Health marker before the Node name, then `Head`, `Peers` with inbound/outbound counts, and `Oldest component update`. The latter is the earliest Server receipt across RPC, Sync, and Consensus and is `Unknown` when the complete timestamp is unavailable; it shows relative time plus a complete UTC absolute time and a visible explanation. A quiet inline RPC/Sync/Consensus collection-state row follows, routine Healthy prose is omitted, exceptional Server-owned reasons wrap without truncation, and `View details →` navigates to Node Detail. Network Overview cards may contain nested Node links; they are not the root dashboard's whole-card-only contract.
...
1. The page identity is an uncarded heading block rather than a hero card. It shows the Network back context, a two-state Server-owned Node Health marker before the display name and Node ID, the exceptional health reason, and the last Agent report time directly, over the shared top atmosphere as in the accepted A calibration — no bordered hero surface, no coloured top/edge strip, and no container wrapping the observations that follow. Node Validator Activity is not rendered by the current SPA, so the identity block carries neither a `Node status` label nor an activity badge. Routine Healthy prose is omitted, while exceptional Server-owned health reasons remain visible.
2. A key summary group presents current `Head`, the sync collection state with progress asserted only from a high-confidence Server Observed Network Head, the current Peer Count, and PlatON process uptime. It renders as four summary tiles — the accepted A calibration's container — that fall from four to two columns with the viewport, and each tile keeps its own honest `Unknown`/last-good text and explanation instead of a fabricated value.
3. Three parallel observation panels form the accepted A layout, each its own card: (1) Node chain and consensus — `QC`/`Locked`/`Committed`, `Validator` membership, resync, reference context; (2) PlatON process resources together with the Node Data directory — `CPU`, `Memory`, process `Started`, process state, and the directory percentage plus used / hosting-filesystem-capacity bytes, never whole-Host disk usage; (3) the shared Host aggregate resources — `Host CPU`/`Host memory`/`Host storage`, `Host upload`/`Host download`, explicitly labelled as collected once per Agent and shared by every Node it monitors. Each unit keeps the data item at the left, the value flush right, and a track under the value line when a valid percentage exists; the panels sit side by side at desktop width and stack in reading order below 48rem. Process uptime is never Host uptime; process CPU/memory are never substituted by the shared Host CPU/memory. Missing or uncertified values remain `Unknown`.
4. The page reads continuously; there is no Details/Network tab control. The four summary tiles, the three observation panels, and the chart section share one vertical reading order, and the deeper diagnostics are native keyboard/touch-operable disclosures with visible `focus-visible` feedback.
5. The chart section is one labelled real latest-60-second window whose bounds come from the existing metrics response (`from`, `to`, `windowSeconds`) and whose series come from retained samples. The six-chart contract is process CPU %, process memory %, shared Host upload/download rates, Peer inbound/outbound count, block interval, and transactions per block, in that order. The first four are line charts and the final two are bar charts; direction series carry readable labels/legends/units and are distinguished by blue/cyan plus text, never colour alone. Desktop lays the deck out three columns by two rows; phones use one column, and tablets fall to two columns while keeping the reading order.
6. Every card keeps its current value, its unit, and the labelled 60-second chart with `60s`/`0s` bounds. `Process CPU` and `Process memory` are the PlatON process percentages (0-100) sampled from the existing history response, `Host network` renders upload/download as distinct line series, `Peer connections` renders inbound/outbound as distinct line series, and `Block interval` and `Transactions per block` use retained samples as bars. Process CPU/memory are never substituted by the shared Host CPU/memory; reduced padding, chart height, radius and shadow keep the deck close to Komari's information density.
7. The Server may include one last-good point immediately before the window as a line chart's starting value, but bar charts render only observations inside the window; neither Server nor WebUI fabricates intermediate samples or substitutes zero for unavailable data. One failed or empty series degrades only its own chart: the other charts and every retained current value stay visible.
8. Details does not render Bounded Block History, History Gaps, public Validator analytics, or history export. The separate two-summary history request is used only for the current block-interval label; two missing or non-consecutive summaries produce `Unknown`, never a fabricated zero. Missing or failed metric history leaves the current card value intact and renders an explicit chart state.
9. The Peer diagnostics disclosure presents the selected Node's Public Peer Insight and selected-Node aggregate Peer History; the low-frequency technical disclosure presents Node ID, Network key, component states, high watermark, and reference context only from the Public Projection. Network Overview presents network-level Peer Insight separately. Neither surface exposes peer addresses or a peer identity list, and important errors stay visible above the disclosures.
...
- At 1280x800, Home puts the 2x2 statistics block beside the Peer country map in the compact overview (about 46:54) and uses a Node grid that fits as many 300px cards as the content width allows — three columns at this width, and four at the 82rem desktop content width. Node Detail uses the current Home content width, lays its four summary tiles in one row and its three observation panels side by side in one row, and lays the six equal metric cards in a three-by-two deck; acceptance checks actual readable width rather than a fixed legacy cap.
- At 768x1024, Home keeps the 2x2 statistics, places the Peer country map above them in reading order, and uses a two-column Node grid at this content width. Node Detail's four summary tiles fall to two columns, its three observation panels stack to one column in reading order, and its six equal-height metric cards fall to a two-column deck.
- At 360x800 and 390x844, Home keeps the compact 2x2 statistics, places the compact map above them in reading order, uses a single-column Node grid, and keeps every metric a single full-width `data item / value` row — the metric groups pair only from 30rem, so `Head / Txs / Peers` and the consensus units all stay single-column here. Filter pills scroll within their own control rather than causing page overflow. Node Detail keeps the four summary tiles in a two-column block, stacks the three observation panels single-column with every unit on its own data-item/value row, stacks the six equal-height metric cards, and keeps the disclosure summaries full-width touch controls. At 768x1024 the page composition may be single-column, but the shared table-to-priority-card transform begins below 47.9rem; Admin tables therefore retain their table/local-wrapper behavior at the 768 fixture.
...
- Whole-card Node links, Node Detail Network links, the Network back link, and the Node Detail disclosures are reachable by keyboard in a predictable order. Browser back/forward preserves route context.
- The public observation panels are static labelled containers, not tabs: they add no tab/tablist semantics, and the only disclosure controls are the native Node Detail `details`/`summary` regions, which keep keyboard activation, visible focus, and descriptive summary text.
...
The production theme lifecycle is a shared foundation, not a per-page option. It applies to public Home, Network Overview, the public Node Detail, Login, Admin direct entry, and reloads. Issue #146 delivered the lifecycle, the shared foundation, and the Login adaptation; Issue #147 completed the public Home, Network Overview, and Node Detail adaptation as a visual-only slice; Issue #148 completed the Admin workbench and management-flow adaptation on the same foundation. Admin keeps its quiet undecorated workbench background and never adopts the public grid.
...
- **Public Home, Network, and Node Detail (Issue #147).** The three public surfaces complete the shared foundation without changing routes or information structure. The Home 2x2 statistics, the Network filter/sort controls and Peer/Geo/Validator modules with their independent degradation, the Peer country map scope/denominator/states, the Server-owned Node Health Summary (Healthy green, every other state grey, with the abnormal reason in text), the last-good semantics, and the Node Detail grouped info and six-chart deck behave exactly as before; only the visual adaptation changes.
  - Information cards use the exact default Emerald surface and typography contract under "Visual language" above. Home Node cards alone show the 20px glow plus 1px shadow ring in emerald-600 at 10% and a 2px lift, with 200ms transitions. Home statistics, Network information cards, Node Detail summary/observation panels, charts, and diagnostic disclosures become opaque without glow or lift. Static cards gain no click handler, pointer cursor, or tab stop; reduced motion removes non-essential transitions and the lift. Whole-card Home Node links retain focus highlight parity, and interactive elements keep their independent visible keyboard focus outlines.
...
| `SCN-THEME-LIFECYCLE` | Auto → Light → Dark cycle and accessible names; refresh persistence; live system change under Auto; explicit Light/Dark override; invalid/unavailable storage; pre-mount first paint on direct entry; production key isolated from the prototype key; Login, Home, Admin, Network Overview, and the public Node Detail readable in both themes on the shared full-bleed Emerald top atmosphere with keyboard focus, 44×44 targets, no page overflow, and reduced motion; public card feedback (60% at rest and opaque on hover-capable devices; only Home Node cards glow/lift, statistics and detail/chart/disclosure cards stay shadow-free and stationary; reduced motion removes the lift); Home filters/sorting remain operable in Dark (see §11.1) |
...
| Public Home, Network Overview, and current Node Detail dual-theme adaptation with the calibrated ~60% card surfaces and hover/focus/disclosure feedback | Issue #147, child of Issue #145; visual-only slice of the #146 foundation |
