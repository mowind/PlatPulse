...
1. One compact Komari-inspired Node card owns the page identity. It uses a quiet neutral outline, restrained radius and shadow, no coloured top/edge strip, and shows the Network back context, a two-state Server-owned Node Health marker before the display name/Node ID, the exceptional health reason, and the last Agent report time directly. Node Validator Activity is not rendered by the current SPA, so the hero carries neither a `Node status` label nor an activity badge. Routine Healthy prose is omitted, while exceptional Server-owned health reasons remain visible.
2. A key summary group presents current `Head`, the sync collection state with progress asserted only from a high-confidence Server Observed Network Head, the current Peer Count, and PlatON process uptime. It uses the same container-sized metric-cell grid as the other groups, so unavailable or complex states are not forced into four equal numeric boxes.
3. The observation groups share that container-sized cell grid: chain and consensus (`QC`/`Locked`/`Committed`, `Validator` membership, resync, reference context); PlatON process resources (`CPU`, `Memory`, process `Started`, process state); the Node Data directory (percentage plus used / hosting-filesystem-capacity bytes, never whole-Host disk usage); and the shared Host aggregate resources (`Host CPU`/`Host memory`/`Host storage`, `Host upload`/`Host download`) explicitly labelled as collected once per Agent and shared by every Node it monitors. Each unit keeps the data item at the left, the value flush right, and a track under the value line when a valid percentage exists; a group with a single unit stays one cell width and left-aligned instead of stretching. Process uptime is never Host uptime; process CPU/memory are never substituted by the shared Host CPU/memory. Missing or uncertified values remain `Unknown`.
...
- At 768x1024, Home keeps the 2x2 statistics, places the Peer country map above them in reading order, and uses a two-column Node grid at this content width. Node Detail's hero groups fall to two metric cells per row, and its six equal-height metric cards fall to a two-column deck.
- At 360x800 and 390x844, Home keeps the compact 2x2 statistics, places the compact map above them in reading order, uses a single-column Node grid, and keeps every metric a single full-width `data item / value` row — the metric groups pair only from 30rem, so `Head / Txs / Peers` and the consensus units all stay single-column here. Filter pills scroll within their own control rather than causing page overflow. Node Detail keeps the process/Node Data/Host metric groups in compact rows, single-column at these widths, stacks the six equal-height metric cards, and keeps the disclosure summaries full-width touch controls. At 768x1024 the page composition may be single-column, but the shared table-to-priority-card transform begins below 47.9rem; Admin tables therefore retain their table/local-wrapper behavior at the 768 fixture.
