...
- **Public Home, Network, and Node Detail (Issue #147).** The three public surfaces complete the shared foundation without changing routes or information structure. The Home 2x2 statistics, the Network filter/sort controls and Peer/Geo/Validator modules with their independent degradation, the Peer country map scope/denominator/states, the Server-owned Node Health Summary (Healthy green, every other state grey, with the abnormal reason in text), the last-good semantics, and the Node Detail grouped info and six-chart deck behave exactly as before; only the visual adaptation changes.
  - Information cards (the Home statistics and Node cards, the Node Detail summary tiles and observation panels, and the Peer/Geo and Network cards — the shared Network `node-card` container is used by both Node and Validator cards) keep the calibrated ~60% surface at rest and become opaque with a faint Emerald outline/glow and about a 2px lift on hover-capable devices only. Chart and diagnostic-disclosure cards gain the outline/glow without moving the plot. Static cards gain no click handler, pointer cursor, or tab stop; reduced motion removes the lift, and focused cards keep an independent visible focus ring.
