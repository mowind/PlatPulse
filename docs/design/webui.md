...
5. Compact Active Node cards use natural heights without stretching shorter cards to a neighbour. A stretched semantic Node-name link opens Node Detail; copy/Details controls remain independent buttons, never nested in the link. The two-state Server-owned Node Health marker stays before the Node name. The header Node Role expresses consensus membership, separate from Linked Validator staking validity and Provider data freshness. The Network name and process uptime share a compact secondary line; identity details remain keyboard/touch-accessible. Healthy Nodes omit routine prose; exceptional Nodes may show one short diagnostic line.
6. Card data ownership is explicit: PlatON process CPU/Memory, Node data size/capacity, shared Host transfer rates, Node Head/Txs/Peers, and Consensus QC/Locked/Committed/membership. Resource rows retain their progress and used/capacity explanations. Head/QC/Locked/Committed use normal-size stacked label/value cells in two columns even on phones; only exceptionally long full-number values over 15 characters span both columns instead of shrinking, clipping or splitting digits. Txs/Peers retain their compact pair with a separate full-width fallback only for unusually large counts. Linked Validator metrics use six directly visible stacked cells in two columns by three rows, with only the strict all-missing/currently-confirmed-Not-Validator exception in the [Validator presentation contract](validator-metrics.md#accepted-node-presentation-target-and-edge-cases). A readable identity fingerprint, full-identifier copy, and accessible Details retain exact values/provenance/reasons. Missing values remain explicitly Unknown rather than zero; historical values, real zero, Unranked, Not applicable and independent stale/error dimensions must survive compaction.
...
- At 1280x800 and wider desktop viewports, Home retains its 1280px page cap, places the six statistics (three columns by two rows) beside the contained 2:1 map in about 4:3 tracks, which keeps each tile at the original four-card width, and fits Node columns using the existing auto-fill/minimum-300px rule; the overview does not redefine Node-card breakpoints or widen the page. Node cards have natural independent heights. Node Detail retains its four summary tiles, three observation panels and three-by-two chart deck.
- At 768x1024, Home puts its six statistics in three columns by two rows above the proportional map and keeps the existing two-column Node grid. Node Detail’s four summary tiles fall to two columns, its three observation panels stack in reading order, and its six equal-height metric cards fall to two columns.
- At 360x800 and 390x844, Home puts the six statistics in two columns by three rows above the map, preserving DOM order, and uses a single-column Node grid. Normal chain and Linked Validator metrics keep their two-column stacked cells; rare oversized full numerals use a full-width cell without clipping or reduced type. Filter pills scroll within their control, not the page. Node Detail retains its two-column summary block, single-column observation panels and six-chart stack with touch-operable disclosures. The Admin table-to-priority-card transform still begins below 47.9rem; Admin tables therefore retain their local-wrapper behavior at the 768 fixture.
