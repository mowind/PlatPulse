...
3. A chain-specific consensus summary presents the parallel heights `Head / QC / Locked / Committed` and an independent `Validator` role; explicit membership renders `Yes` or `No`. Process start time and last Agent report time form the card footer. Missing or uncertified values remain `Unknown`.
