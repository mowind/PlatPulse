...
│   └── Node list/cards
└── Node Detail
    ├── Two-state Node Health marker, Node status, Validator membership role, and process uptime
...
5. Compact Active Node cards. Each whole root-dashboard card is one semantic link to Node Detail; a two-state Server-owned Node Health marker sits before the Node name, the Network name is plain text and there is no nested Network link. Healthy Nodes omit routine prose; exceptional Nodes may show one short diagnostic line.
6. Card data ownership is explicit: PlatON process `CPU`/`Memory`, `Node data` size/capacity, Host transfer rates (`↑ Up`/`↓ Down`), Node `Head`/`Txs`/`Peers`, and Consensus `QC`/`Locked`/`Committed`/`Validator`. Every metric is one full-width row: the data item sits at the left and the value sits flush right at every breakpoint; a row with progress puts a full-width track under the value line with its explanation below the track, and a row without progress puts its explanation directly under the value. `Node data` reads as a percentage whose explanation gives the used / filesystem-capacity bytes. Missing values remain unavailable rather than becoming zero.
...
- At 1280x800, Home puts the 2x2 statistics block beside the Peer country map in the compact overview (about 42:58) and uses a two-column Node grid. Node Detail uses the current Home content width, stacks its resource and height metric rows in one column at every breakpoint, and lays the four equal metric cards in a two-by-two deck; acceptance checks actual readable width rather than a fixed legacy cap.
...
- At 360x800 and 390x844, Home keeps the compact 2x2 statistics, stacks the compact (expandable) map below them, uses a single-column Node grid, and keeps every metric a single full-width `data item / value` row (`Head / Txs / Peers` and the consensus rows included). Filter pills scroll within their own control rather than causing page overflow. Node Detail keeps the three resource metrics and three status facts in compact rows with every metric row single-column at every breakpoint, stacks the four equal-height metric cards, and keeps the tabs full-width touch controls. At 768x1024 the page composition may be single-column, but the shared table-to-priority-card transform begins below 47.9rem; Admin tables therefore retain their table/local-wrapper behavior at the 768 fixture.
