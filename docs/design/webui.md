...
- Home Dashboard: read-only Public Projections, Node → Node Detail, a compact overview combining the global statistics with the Peer country map, and a current block interval derived from the latest two consecutive retained Block Summaries;
...
1. The page identity is an uncarded heading block rather than a hero card. It shows the Home `All Networks` back context, a two-state Server-owned Node Health marker before the display name and Node ID, the exceptional health reason, and the last Agent report time directly, over the shared top atmosphere as in the accepted A calibration — no bordered hero surface, no coloured top/edge strip, and no container wrapping the observations that follow. Node Validator Activity is not rendered by the current SPA, so the identity block carries neither a `Node status` label nor an activity badge. Routine Healthy prose is omitted, while exceptional Server-owned health reasons remain visible.
...
The fixed acceptance viewports are 360x800, 390x844, 768x1024, and 1280x800. Long state, attribution, and identity text wraps without horizontal overflow, and keyboard focus and touch targets remain usable in every project.
...
- **Public Home and Node Detail (Issue #147).** The two public surfaces complete the shared foundation without changing routes or information structure. The Home 2x2 statistics, the Network filter/sort controls, the Peer country map scope/denominator/states, the Server-owned Node Health Summary (Healthy green, every other state grey, with the abnormal reason in text), the last-good semantics, and the Node Detail grouped info and six-chart deck behave exactly as before; only the visual adaptation changes.
  - Information cards use the exact default Emerald surface and typography contract under "Visual language" above. Home Node cards alone show the 20px glow plus 1px shadow ring in emerald-600 at 10% and a 2px lift, with 200ms transitions. Home statistics, Node Detail summary/observation panels, charts, and diagnostic disclosures become opaque without glow or lift. Static cards gain no click handler, pointer cursor, or tab stop; reduced motion removes non-essential transitions and the lift. Whole-card Home Node links retain focus highlight parity, and interactive elements keep their independent visible keyboard focus outlines.
