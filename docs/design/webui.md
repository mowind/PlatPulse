...
1. The page identity remains an uncarded heading block, with the Home `All Networks` back context, the two-state Server-owned Node Health marker before the display name, and the exceptional health reason — no bordered hero, coloured edge strip or observation wrapper. The Node ID keeps its full UUID but carries no more weight than the other secondary identity text, so the display name stays the heading. Routine Healthy prose is omitted. The title area shows Node Health plus Activity only. The Linked Validator overview header independently presents the `Validator` badge (Current Validator Status / valid staking identity), the `Current` cue (Provider freshness/state), and `Activity` (Validator Provider activity). A decided staking verdict keeps a badge, but the non-verdict `Validator status unknown` does not: an unestablished validity must not present as a third equally weighted chip, so it becomes one muted line under the header carrying the existing canonical explanation plus a touch- and keyboard-operable info control whose text restates it in plain language. None is inferred from either of the others or from Node Health; consensus membership remains in Chain info. Reuse the existing Activity badge semantics and accessible source/time/reason explanation in both the title area and Linked Validator overview, including Observing for unavailable evidence with its actual reason; preserve canonical last-good activity and stale annotation rather than inventing a new mapping.
2. Keep four summary cards: current `Head`, Sync, Peer Count and PlatON process uptime; four columns at `lg` (1024px) and above, two below. Head adds a signed `Head delta = Node Head - Observed Network Head`: negative means behind, positive ahead, zero equal. Compute it only with a current valid Node Head and a current valid, high-confidence Server Observed Network Head for the same Network. Missing, invalid, stale or low-confidence input renders a dash plus the specific reason, never zero or a last-good delta. The existing Public Node `freshness` field is an oldest-receipt timestamp, not a freshness enum. Without an API expansion, the comparison conservatively requires a valid receipt timestamp, `rpcState=ok` and Server `health=healthy` (which confirms current RPC/sync/consensus). Other health outcomes do not prove Head currency independently, so omit the delta with an explicit reason while retaining absolute observations; never infer a new frontend freshness threshold or change Activity. Sync progress likewise requires the high-confidence Server reference; it is not redefined by this display delta. Both tiles name the `Observed Network Head` in full — the page must not shorten the canonical term to a bare "head" or "network reference" while a second, different reference is also on screen. One height phrase family serves both: an exact match reads `At Observed Network Head` rather than a bare zero, and an offset reads `+2 from Observed Network Head` or `−2 from Observed Network Head`. Sync states its direction in words — `2 blocks behind Observed Network Head`, `At Observed Network Head`, `2 blocks ahead of Observed Network Head` — so a Node ahead of the reference is never reported as being at it. Each card keeps its own honest Unknown/last-good explanation.
3. Keep the three observation panels: chain/consensus; PlatON process resources and Node Data; shared Host resources collected once per Agent. Preserve current fields, valid percentage tracks, and the distinction between process memory/uptime and Host memory/uptime, and between Node Data and whole-Host storage. Panels remain side by side at `lg` (1024px) and above and stack below `lg`. `QC`, `Locked` and `Committed` show their absolute heights plus an offset against the `Node Head` — the reference is named explicitly, because the summary cards above compare against the different `Observed Network Head` — and only when both inputs are current and valid. An exact match reads `At Node Head`, not a bare zero; otherwise the offset reads `+2 from Node Head` / `−1 from Node Head`. Stale last-good consensus heights keep their annotated absolute value but no offset; unavailable/invalid/stale comparison inputs show a dash with a reason, never a fabricated zero.
4. The Linked Validator overview exposes all six metrics: Validator Lifetime Block Count, Validator Lifetime Rewards, Validator Rank, Validator Block Production Completion Rate, PlatScan 24-Hour Block Production Rate and Effective Delegation Reward Distribution Ratio. Those names identify the metrics; the displayed captions stay the short names in [the Validator presentation contract](validator-metrics.md#accepted-node-presentation-target-and-edge-cases) (`Cumulative blocks`, `Cumulative rewards`, `Network rank`, `Production rate`, `PlatScan 24h rate`, `Delegation reward share`), which allow short display names provided each keeps its full field name as an accessible name and in Details. Use two shrinkable columns below `lg`, three at `lg` and above; preserve reading order, complete integer counts and all available source reward precision without abbreviating, rounding through binary floating point or reconstructing absent digits. Keep existing percentage formatting and source/not-applicable explanations. Ordinary noncompact metric captions wrap naturally rather than ellipsizing or reserving a compact one-line slot. A value too long for one column takes the whole row rather than being folded into a narrow column; its fractional digits may be de-emphasised in colour so the magnitude reads first, but every digit stays present, in order, and available to assistive technology. Preserve genuine zero, Unknown, Unranked, not-applicable and last-good states independently.
5. The Node ID keeps its existing full UUID display. Only the linked Validator identifier is abbreviated in its overview position, with an explicit copy-full-identifier control and a keyboard/touch-operable full-identifier reveal (not hover-only). Both controls are icon-only to keep the row narrow on a phone, but each keeps its full accessible name (`Copy full Validator identifier`, `Show full ID` / `Hide full ID`) and carries the same words in a tooltip that works on touch and on focus. Copy always uses the complete identifier. Failure announces the failure and reveals the complete selectable text for manual copying, never a false success; revealed long identifiers wrap. Preserve accessible control names, visible focus and 44×44 targets.
...
7. The page reads continuously without Details/Network tabs. Node Detail has exactly three collapsible regions — Validator diagnostics, Peer diagnostics, and Identifiers and technical details — and all three are one shared disclosure component, not three implementations: same height, padding, chevron, focus ring, title weight, and `11px` description line under the title. They use native `details`/`summary`, default collapsed, with keyboard/touch operation and visible focus. The Validator diagnostics sit inside the Linked Validator card they explain, so they take the shared presentation without painting a second card in a card. Component states, Provider states and detailed provenance (source, successful update time, source cutoff when available and unavailable/degraded reasons) remain available inside the relevant disclosure. Important independent errors, exceptional health reasons and relevant stale/degraded warnings remain visible beside the affected summary/panel/Linked Validator overview while collapsed; hiding diagnostics must not hide a Provider failure, rank failure, identity problem or unrelated collection error. Only authorized Public Projection evidence is exposed, never Peer addresses or a Peer identity list.
...
10. Node Detail summary cards, observation panels, Linked Validator overview, chart cards and diagnostic surfaces are static: retain their resting Emerald surface opacity with no opacity change on hover, decorative glow or lift. They are ordered into three reading tiers using only opacities Emerald or the existing tree already carry — summary cards `bg-background/60`, observation panels, the Linked Validator overview and chart cards `bg-background/50`, collapsed disclosures `bg-background/40`. Five points of background opacity per step is the whole separation: it must read as depth, not as differently coloured blocks, and no new colour token is introduced. Disclosure triggers, links, copy/reveal controls and other actual controls keep interactive feedback and visible focus. Home Node cards use an explicit interactive-card treatment and retain their existing whole-card navigation/hover/focus behavior; do not apply the Node Detail change globally to Home statistics, Admin, Login or other pages.
...
- Verify all six exact Linked Validator metrics, two columns at 360/390/768 and three at 1280/1440, wrapping noncompact captions, long IDs/values and full-precision rewards without clipping. Check independent Validator (valid staking)/Current (Provider freshness/state)/Activity in the Linked Validator header, Health plus Activity only in the title, and consensus membership in Chain info; preserve existing Activity semantics in both placements. Confirm the non-verdict `Validator status unknown` carries no badge, keeps its canonical explanation with a touch/keyboard-operable info control, and that the two identifier controls are icon-only with full accessible names on 44×44 targets.
...
- Check the Observed Network Head offsets behind/equal/ahead and missing/invalid/stale/low-confidence references; check that Sync distinguishes behind, level and ahead rather than calling every non-behind Node level; check absolute QC/Locked/Committed with their `At Node Head` / `from Node Head` offsets, with no stale-input offset.
...
- Verify one over-long exact reward takes the whole metric row instead of a narrow column, keeps every digit in the DOM in order, and leaves the six-cell grid intact at 360/390/768/1280/1440.
- Confirm the same disclosure component renders all three regions with identical height, padding, chevron, focus ring and title/description typography, and that all three start collapsed.
- Measure both plot and SVG heights (7.25rem at 360/390/768, 6.25rem at 1280/1440), unchanged six-chart order/grid and retained 60-second samples. Verify static Node Detail hover does not change opacity, that the three surface tiers stay within five points of background opacity per step, and that interactive controls plus Home Node-card feedback remain intact; other pages and the 1280px shell are unchanged.
