...
- the page is one continuous reading surface in the order: back navigation and Node identity (two-state Server-owned Node Health marker, display name, Node ID, abnormal health reason, last Agent report time); key summary; grouped observations; the labelled real latest-60-second chart section; collapsible Peer diagnostics; collapsible low-frequency identifiers and technical details. The former Details/Network tabs are removed rather than kept as a competing layout;
- the key summary presents current `Head`, sync state and progress from the Server Observed Network Head with its confidence, current Peer Count, and PlatON process uptime. Progress is asserted only from a high-confidence reference; a low or unknown confidence is shown as context, never as authoritative progress;
- observation groups cover Node chain/consensus (`QC`/`Locked`/`Committed`, `Validator` membership, resync, reference context), PlatON process resources (`CPU`, `Memory`, process start, process state), the Node Data directory (its percentage and used / hosting-filesystem-capacity bytes, never whole-Host disk usage), and the shared Host aggregate resources (CPU, memory, storage, and sampled upload/download rates) explicitly labelled as collected once per Agent and shared by every Node it monitors. Process uptime is never Host uptime, and process CPU/memory are never substituted by Host CPU/memory;
- the chart section is one labelled real latest-60-second window. This issue (#149) carries the existing four equal-size one-minute metric cards - shared Host network upload/download, Peer connections, block interval, and transactions per block - as a running transition to the final six-chart target of the parent specification; Network and Connections use line charts and Block time and Transactions use bars. Missing or failed metric history leaves every current value intact and renders an explicit per-chart state;
...
1. One compact Komari-inspired Node card owns the page identity. It uses a quiet neutral outline, restrained radius and shadow, no coloured top/edge strip, and shows the Network back context, a two-state Server-owned Node Health marker before the display name/Node ID, the separate `Validator` membership role, and process uptime. Node Validator Activity is not rendered by the current SPA, so the hero carries neither a `Node status` label nor an activity badge. Routine Healthy prose is omitted, while exceptional Server-owned health reasons remain visible.
2. A compact resource list inside that card uses the same metric grouping as Home. Because the hero is far wider than a Node card, a group fills itself with as many ~17rem metric cells as the container holds rather than stretching a fixed pair across the page: at the Home content width the three resources `CPU`, `Memory`, and `Node data` share one row. Each unit keeps the data item at the left, the value flush right, and a track under the value line when a valid percentage exists; `Node data` reads as a percentage and its explanation gives the used / filesystem-capacity bytes below the track; unavailable values remain explicit rather than becoming zero. A group with a single unit, such as `Validator`, keeps one cell width and stays left-aligned instead of stretching.
3. A chain-specific consensus summary presents the parallel heights `Head / QC / Locked / Committed` and an independent `Validator` role as the same metric units, laid out in the same container-sized cell grid — one row for the four heights at the Home content width, and one full-width row on a phone. `Validator` stays in the same chain group as the heights rather than behind a second dividing rule; explicit membership renders `True` or `False`. Process start time and last Agent report time form the card footer. Missing or uncertified values remain `Unknown`.
4. The page reads continuously; there is no Details/Network tab control. The key summary, the grouped observations, and the chart section share one vertical reading order, and the deeper diagnostics are native keyboard/touch-operable disclosures with visible `focus-visible` feedback.
5. The chart section is one labelled real latest-60-second window whose bounds come from the existing metrics response (`from`, `to`, `windowSeconds`) and whose series come from retained samples. The final six-chart contract is process CPU %, process memory %, shared Host upload/download rates, Peer inbound/outbound count, block interval, and transactions per block, in that order; the first four are line charts and the final two are bar charts. Direction series carry readable labels/legends/units and are distinguished by blue/cyan plus text, never colour alone. Desktop lays the deck out three columns by two rows; phones use one column, and tablets adapt while keeping the reading order. The four-chart deck below is the running transition that this issue (#149) delivers before #150 adds the two process charts.
6. In the current transition, the deck presents four equal-size, compact cards in this order: Host network upload/download, Peer connections, block interval (`latest block timestamp - previous consecutive block timestamp`), and latest Block Summary transaction count. Every card keeps its current value and a labelled 60-second chart with `1m` and `0s` time bounds; reduced padding, chart height, radius and shadow keep the deck close to Komari's information density. Network/Connections render upload/download and inbound/outbound as distinct line series, Block time and Transactions as bars.
7. The Server may include one last-good point immediately before the window as a line chart's starting value, but bar charts render only observations inside the window; neither Server nor WebUI fabricates intermediate samples or substitutes zero for unavailable data. One failed or empty series degrades its own chart without blanking the others.
...
- At 1280x800, Home puts the 2x2 statistics block beside the Peer country map in the compact overview (about 46:54) and uses a Node grid that fits as many 300px cards as the content width allows — three columns at this width, and four at the 82rem desktop content width. Node Detail uses the current Home content width and fills each hero group with as many ~17rem metric cells as that width holds, so the three resources and the four heights each occupy one row, and lays the four equal metric cards in a two-by-two transition deck; acceptance checks actual readable width rather than a fixed legacy cap.
- At 768x1024, Home keeps the 2x2 statistics, places the Peer country map above them in reading order, and uses a two-column Node grid at this content width. Node Detail's hero groups fall to two metric cells per row, and its four equal-height metric cards keep their two-by-two transition deck.
- At 360x800 and 390x844, Home keeps the compact 2x2 statistics, places the compact map above them in reading order, uses a single-column Node grid, and keeps every metric a single full-width `data item / value` row — the metric groups pair only from 30rem, so `Head / Txs / Peers` and the consensus units all stay single-column here. Filter pills scroll within their own control rather than causing page overflow. Node Detail keeps the process/Node Data/Host metric groups in compact rows, single-column at these widths, stacks the four equal-height metric cards, and keeps the disclosure summaries full-width touch controls. At 768x1024 the page composition may be single-column, but the shared table-to-priority-card transform begins below 47.9rem; Admin tables therefore retain their table/local-wrapper behavior at the 768 fixture.
...
- **Public Home, Network, and Node Detail (Issue #147).** The three public surfaces complete the shared foundation without changing routes or information structure. The Home 2x2 statistics, the Network filter/sort controls and Peer/Geo/Validator modules with their independent degradation, the Peer country map scope/denominator/states, the Server-owned Node Health Summary (Healthy green, every other state grey, with the abnormal reason in text), the last-good semantics, and the Node Detail grouped info and four-chart deck behave exactly as before; only the visual adaptation changes.
  - Information cards (the Home statistics and Node cards, the Node Detail hero, and the Peer/Geo and Network cards — the shared Network `node-card` container is used by both Node and Validator cards) keep the calibrated ~60% surface at rest and become opaque with a faint Emerald outline/glow and about a 2px lift on hover-capable devices only. Chart and diagnostic-disclosure cards gain the outline/glow without moving the plot. Static cards gain no click handler, pointer cursor, or tab stop; reduced motion removes the lift, and focused cards keep an independent visible focus ring.
  - The public top atmosphere, SVG grid geometry, and layered fade stay pointer-inert and viewport-fixed in both themes; the Node Detail info groups and disclosures keep the shared card surface, hairline separators, and chart furniture readable in Dark. The transparent map uses a quiet slate basemap in Dark with the same Emerald observed-country fill and marker treatment, and Dark keeps group hairlines, chart grid lines, the secondary violet series, and Emerald link hover readable rather than inverting them.
...
| `SCN-HOME-NODE-DETAIL` | compact Komari-density Node card with no coloured edge strip, compact PlatON process CPU/process memory/Node Data progress, the Validator role and process uptime but no Node-status label or activity badge, four equal-height compact detail cards containing two 60-second line charts and two bar charts backed by real retained samples, neutral card borders, no rendered Bounded Block History, derived consecutive-block interval, and selected-Node aggregate Peer History in the Network tab |
...
| Public Home, Network Overview, and current Node Detail dual-theme adaptation with the calibrated ~60% card surfaces and hover/focus/disclosure feedback | Issue #147, child of Issue #145; visual-only slice of the #146 foundation, keeping the four-chart deck |
