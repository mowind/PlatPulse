...
- IPinfo 票（#134）不含 GeoJS、全局强制刷新与第三方 MMDB 自动下载；GeoJS 票（#135）复用同一条后台执行路径与 provider 隔离把它接通。**交接（issue #136 全局强制刷新）：** 该票尚未合入，本票不渲染不存在的刷新按钮。合入时不需要新增 provider 分支：强制重查应把目标地址送进同一条 `geo_backfill` 执行路径（`run_pass` 及共享的调度、去重、generation 校验、缓存写入），由当前选中 provider 的 `ExternalGeoClient` 或本地 MMDB 读取器解析，因此 IPinfo 与 GeoJS 自动获得相同行为；计数与部分失败仍按本节的 Provider 隔离与 last-good 规则呈现。浏览器 E2E 只验证两个外部 Provider 选项与各自的知情说明存在且默认未选，绝不在真实浏览器会话里选中它们：E2E 用的 Server 没有（也不允许有）可配置目的地，选中即意味着把 Peer 地址发给真实第三方，违反"CI 不向供应商发送真实 Peer IP"。外发行为（成功、无国家、畸形响应、HTTP 失败、429、超时、恢复、慢响应、切换与迟到结果）由进程内的确定性替身覆盖。
