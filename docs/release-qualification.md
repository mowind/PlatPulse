...
Run `scripts/final-release-qualification.sh --profile release/qualification/ci.toml --require-all` for the final Phase 5 gate (use `soak.toml` for the scheduled soak profile). The GitHub workflow provisions Rust format/lint components, `cargo-deny`, `cargo-audit`, npm dependencies, and the fixed Playwright Chromium browser before invoking the same runner.
